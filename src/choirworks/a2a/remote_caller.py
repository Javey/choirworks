...
class RemoteAgentCaller:
    ...
    async def stream_remote(
        ...
        *,
        ...
        current = await self.consume_chunks(ctx, node, chunks)
        return await self.ensure_terminal(ctx, node, current)
    ...
    async def cancel_remote_task(self, agent_url: str, task_id: str) -> None:
        await self._remote.cancel_task(agent_url, task_id)
