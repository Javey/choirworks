...
plan_flow: Flow[None] = Flow(
    name="plan_flow",
    start=_check_retryable,
    edges=(
        Edge(_check_retryable, _backoff_continue, frozenset({"retry"})),
        Edge(_check_retryable, _check_input_required, frozenset({"next"})),
        Edge(_check_input_required, _exit_wait, frozenset({"wait_input"})),
        Edge(_check_input_required, _check_all_completed, frozenset({"next"})),
        Edge(_check_all_completed, _emit_completed, frozenset({"plan_completed"})),
        Edge(_check_all_completed, _check_repairable, frozenset({"next"})),
        Edge(_check_repairable, _repair, frozenset({"repair"})),
        Edge(_check_repairable, _stalled, frozenset({"next"})),
        Edge(_repair, _backoff_continue, frozenset({"repair_ok"})),
        Edge(_repair, _emit_failed, frozenset({DEFAULT})),
    ),
)
