...
群消息必须符合人类工作群直觉，固定顺序如下（`tests/integration/test_room_ordering.py` 回归锁定）：

1. **CEO 发言**：`#1 [user/CEO] 请求原文`（入群播报绝不早于人类消息）。
2. **协调者拆解**：`任务已拆解：@A 负责…、@B 负责…`（assistant）。
3. **成员入群**：计划选中的 Agent 逐个播报 `已将 @A 加入群聊`（`reason=plan`），此后成员栏可见。
4. **派发**：`已派发 @A：…` → Agent 产出消息（可并行交错）。
5. **协助链路**：`A 的求助原文`（role=agent，A 自己发声）→ `@A 请求 @C 协助，已安排`（协调者决策）→ `已将 @C 加入群聊` → `已派发 @C` → C 产出。
6. **收尾**：`任务完成：…`。

约定：人类 @ 的直派任务 = 用户消息 → 入群 → 派发（无拆解消息）；规划路径 = 用户消息 → 拆解 → 入群 → 派发。入群播报只在首次入群时出现。
